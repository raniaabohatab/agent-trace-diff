...
```bash
python src/generate_traces.py --task "What is 23 * 17, and what is the capital of France?"
```

Runs a 3-tool (calculator, search, read_file) LangChain agent on `claude-haiku-4-5` and
prints each step to the console. Tools return deterministic, canned data — no real
external APIs — so runs are repeatable while the schema and pipeline are being built.
