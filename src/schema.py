...
class Step(BaseModel):
    ...
    planned_tool: str | None = None  # tool name the agent said it would call, if step_type == "plan"
    actual_tool: str | None = None  # tool name actually invoked, if step_type == "action"
    tool_input: dict | None = None  # arguments passed to the tool
    tool_output: Any | None = None  # raw output/observation returned
    raw_thought: str | None = None  # the agent's reasoning text for this step, if captured
    timestamp: str  # ISO 8601
    tool_call_id: str | None = None  # links an action to its observation when several run in parallel, Week 7+
...
class AgentRun(BaseModel):
    ...
    ground_truth_divergence_step: int | None = None  # filled in manually in Week 5, leave None for now
    injected_failure: str | None = None  # "wrong_tool" | "skip_step" | "corrupt_args", Week 5 self-constructed cases only
